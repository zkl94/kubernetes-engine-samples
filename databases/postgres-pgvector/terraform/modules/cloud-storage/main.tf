...
# [START gke_databases_postgres_pgvector_cloud_storage_bucket]
...
module "service-account-bucket" {
  ...
  description     = "Service account to access the bucket with Postgres training documents"
}
...
module "project-iam-bindings-bucket" {
  ...
  bindings = {
    ...
    "roles/iam.workloadIdentityUser" = ["serviceAccount:${var.project_id}.svc.id.goog[pg-ns/embed-docs-sa]"] 
  } 
...
# [END gke_databases_postgres_pgvector_cloud_storage_bucket]
